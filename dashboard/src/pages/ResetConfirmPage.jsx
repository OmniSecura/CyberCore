...
export function ResetConfirmPage({ t, lang, navigate, token: initialToken }) {
  ...
  if (success) {
    return (
      <SuccessCard
        title={t('pw_changed_title')}
        sub={t('pw_changed_sub')}
        btnLabel={t('go_login')}
  ...
  return (
    <div className="auth-card">
      <div className="head">
        <div className="eyebrow">{t('eyebrow_new')}</div>
        <h1>{t('new_pw_title')}</h1>
        <div className="sub">{t('new_pw_sub')}</div>
      </div>
      {globalErr && <Alert type="error">{globalErr}</Alert>}
      <form className="form" onSubmit={submit} noValidate>
        {!initialToken && (
          <Input
            id="token" label={t('reset_token_label')} type="text"
            placeholder={t('token_ph')} value={token}
            onChange={e => setToken(e.target.value)} error={errors.token}
          />
        )}
        <PasswordInput
          id="password" label={t('new_pw_label')} lang={lang}
          placeholder={t('pw_min_ph')} value={pw}
          onChange={e => setPw(e.target.value)} error={errors.pw}
          showStrength
        />
        <PasswordInput
          id="password2" label={t('confirm_new_pw')} lang={lang}
